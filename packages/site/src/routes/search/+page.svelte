  ...
  <Masonry class="content-center" tiles={data.products} let:tile={product}>
    ...
    >
      <div class="relative w-full flex-basis-6 min-h-20 max-h-[25rem] rounded-t">
        ...
        />
        <div class="absolute left-0 bottom-0 right-0 top-0 flex flex-col bg-transparent p-4">
          {#if product.price}
            <span class="ml-auto rounded-full px-2 py-0.5 text-sm bg-orange-400 border border-orange-500/50 text-orange-100">{product.price}</span>
          {/if}
          {#if product.brand}
              {#if product.brand.logo}
                ...
                />
              {:else}
                <span class="mr-auto mt-auto rounded px-2 py-0.5 text-sm bg-stone-100 border border-stone-200/50 font-semibold text-stone-700">{product.brand.name}</span>
              {/if}
          {/if}
        </div>
      </div>
