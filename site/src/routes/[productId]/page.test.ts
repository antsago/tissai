...
describe("Product Detail page", () => {
	it("shows product's details", async () => {
		...
		render(page, { data: await load({ params: { productId: PRODUCT.id } } as any) } as any)

		const image = screen.getByAltText(PRODUCT.name)
		const heading = screen.getByRole("heading")
		const description = screen.getByRole("main")
		const buyLink = screen.getByRole("link", { name: (n) => n.includes(PRODUCT.sellers[0].shop.name) })
